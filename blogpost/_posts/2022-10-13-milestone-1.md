...
Par exemple, l'image du widget montre l'événement 230 du premier match de la saison règulière en 2016). Sur le [site officiel de NHL](https://www.nhl.com/gamecenter/tor-vs-ott/2016/10/12/2016020001/recap/plays#game=2016020001,game_state=final,lock_state=final,game_tab=plays), on retrouve les mêmes coordonnées à la période 2 à 02:08 qui correspond à cet événement.
...
4. Pour déterminer un tour du chapeau naturel, c'est-à-dire qu'un joueur marque trois buts consécutifs au cours d'un même match excluant la période shootout, lorsqu'il y a un événement de type "Goal", on peut vérifier, si les deux prochains événements "Goal" ont été marqués par la même personne.

5. Pour déterminer si un but ou un shot est marqué accidentellement par un coup de pied, on peut récupérer les points qui ont une coordonnée -89 > x > 89 puisque cela veut dire que le palet est derrière la ligne de but et la seule façon de considérer ce tir comme un shot ou un but est que s'il a été marqué par le patin d'un joueur.
...
Choisissons d'abords une saison: disons la saison réguliaire de 2018-2019. On extrait le *dataframe* pour cette saison avec `NHLDataManager.get_season_dataframe`. On sépare ensuite les données `SHOT` et les données `GOAL`, et on compte le nombre d'instance de chaque pour chaque type de shots `['Tip-In', 'Wrist Shot', 'Snap Shot', 'Backhand', 'Slap Shot']`. Nous allons aussi garder seulement les 5 types de shots les plus communs.
...
Nous allons maintenant nous intéresser au lien entre la distance (jusqu'au but) d'un tir, et la chance que ce tir soit un but.

Pour le calcul de la distance, il est important de prendre en compte le côté de l'équipe (et donc du but) sur la patinoire. Durant un jeu, chaque équipe commence à gauche ou à droite, et change de côté après chaque période. Le *API* nous retourne une valeur `rinkSide` pour chaque période, et pour chaque équipe (`HOME` et `AWAY`). Il faut donc aussi savoir quel équipes correspondent à `HOME` et à `AWAY`.
...
Puisque nous calculons une *chance* de but, il faut aggréger les données afin d'obtenir un pourcentage de buts (pour un certain nombre de tirs). Nous utilisons `pd.cut()` pour séparer les shots/goals en *bins* de distance 10, ce qui en fait 19 bins de distance 0 à 190.

Enfin, nous pouvons calculer un pourcentage de buts pour chacun de ces *bins*, et le visualiser dans un bar plot (pour chaque saison entre 2018 et 2021).

![Image](../Q4_2_2018_2019.png)
![Image](../Q4_2_2019_2020.png)
![Image](../Q4_2_2020_2021.png)

La situation ne semble pas vraiment changer pour chaque année, mais il est clair qu'on peut détecter un motif ici en dedans de chaque saison: la chance de faire un but décroît avec la distance initialement, mais commence à remonter lorsque la distance est haute. Évidemment on s'attends que lorsqu'un shot est fait de plus loin, il est plus difficile de faire un but. Mais qu'est-ce qui explique que les shots de *très* loin soient si effectifs?

![Image](../Q4_2-EmptyNet.png)

La réponse se trouve avec les buts *Empty Nets*: souvent durant la dernière période, les équipes vont remplacer leur gardien de but par un autre joueur offensif, ce qui leur permet d'avoir une meilleure chance de marquer un dernier but. Par contre, ceci laisse aussi leur propre but vulnérable, et il est possible pour l'équipe adversaire de marquer un but à partir de leur zone défensive. En effet, si nous regardons ce graphique avec le pourcentage de *shots* qui sont des *Empty Net Goals*, nous voyons qu'il est beaucoup plus commun de marquer un but Empty Net de loin que de proche.

### 3. Graphique des types de tirs et des distances
Si maintenant nous examinons la chance but dépendent du type de tir et de la distance, nous pouvons utiliser un *heatmap* pour visualiser ces données, où il y a deux dimensions discrètes d'entrée et une dimension continue de sortie.
...
En examinant le résultat, nous voyons que de très proche (0-20), c'est définitivement les shots *Tip-In* qui sont les plus dangereux. Si on va un peu plus loins (20-70), c'est d'abord les *Snap Shots* et ensuite les *Slap Shots* qui sont les plus effectifs. Encore plus loins dans la région de (110-140), les *Tip-In* et *Slap Shots* ont de bonnes chances. Enfin si on tire complétement de l'autre côté de la patinoire, c'est surtout les *Snap Shots* et aussi les *Wrist Shots* qui marchent le mieux. En conclusion, le type de shot le plus dangereux dépend de la distance jusqu'au but.
