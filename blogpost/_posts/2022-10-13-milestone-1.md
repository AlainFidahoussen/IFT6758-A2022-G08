...
À la fin de ce tutoriel, vous serez en mesure d'obtenir toutes les 
...
'notebooks/Milestone-1/Q1_Acquisition_de_données.ipynb'
...
Par exemple, l'image du widget montre l'événement 230 du premier match de la saison règulière en 2016). 
Sur le [site officiel de NHL](https://www.nhl.com/gamecenter/tor-vs-ott/2016/10/12/2016020001/recap/plays#game=2016020001,game_state=final,lock_state=final,game_tab=plays){:target="_blank"}, 
on retrouve les mêmes coordonnées à la période 2 à 02:08 qui correspond à cet événement.
...
3. Si une pénalité débute à la fin de la 3e période règlementaire, la prolongation qui est censée voir 3 joueurs contre 3 joueurs, aura 4 contre 3.
...
Quelques fonctionnalités supplémentaires déduites du dataframe : 
...
3. Pour déterminer la latéralité des joueurs, à chaque événement tir, on peut récupérer l'information sur la dextérité du tireur dans la section [gameData][players][ID][shootsCatches], vérifier si le point de tir (coordonnées x,y) se trouve à droit ou à gauche du demi-terrain et obtenir de cette façon si les joueurs ont une préférence quant au côté de tir.

4. Pour déterminer un hat-trick naturel, c'est-à-dire qu'un joueur marque trois buts consécutifs au cours d'un même match excluant la période shootout, lorsqu'il y a un événement de type "Goal", on peut vérifier, si les deux prochains événements "Goal" ont été marqués par la même personne.


## Question 4 : Visualisations simples
Nous voulons ensuite faire quelques visualisations simples afin de connaître mieux nos données.

### 1. Graphique des types de tirs
Choisissons d'abord une saison, par exemple la régulière de 2018-2019. 
On extrait le *dataframe* de cette saison avec `NHLDataManager.get_season_dataframe`. 
On sépare ensuite les données `SHOT` et les données `GOAL`, et on compte le nombre d'instances de 
chaque pour chaque type de shots `['Tip-In', 'Wrist Shot', 'Snap Shot', 'Backhand', 'Slap Shot']`. 
Nous allons aussi garder seulement les 5 types de shots les plus communs.

Afin de comparer le nombre de tirs des `GOAL` et `SHOT` pour chaque type de tir, nous pouvons utiliser un `bar plot` où on a une barre pour chaque type de tir, et où on empile les `GOAL` et `SHOT` une par dessus l'autre. Afin de connaître le "niveau de danger" de chaque type de shot, nous allons aussi afficher le pourcentage de goals pour chaque type de shot.

![Image](../Q4_1.png)

Nous voyons que les *Tip-In* sont les plus communs de loin, et que les *Slap Shots* sont probablements les tirs les plus dangereux.

### 2. Graphique des distances
Nous allons maintenant nous intéresser au lien entre la distance (jusqu'au but) d'un tir, et la probabilité que ce tir soit un but.

Pour le calcul de la distance, il est important de prendre en compte le côté de l'équipe (et donc du but) sur la patinoire. Durant un jeu, chaque équipe commence à gauche ou à droite, et change de côté après chaque période. Le *API* nous retourne une valeur `rinkSide` pour chaque période, et pour chaque équipe (`HOME` et `AWAY`). Il faut donc aussi savoir quelles équipes correspondent à `HOME` et à `AWAY`.

On peut donc "standardiser" les coordonnées (x,y) pour que le but soit toujours à droite : il suffit simplement d'inverser les coordonnées pour les shots qui ont `rinkSide="right"`. 

Puisque nous calculons une *probabilité* de but, il faut aggréger les données afin d'obtenir un pourcentage de buts (pour un certain nombre de tirs). Nous utilisons `pd.cut()` pour séparer les shots/goals en *bins* de distance 10, 
ce qui en fait 19 bins de distance 0 à 190 ft.

Enfin, nous pouvons calculer un pourcentage de buts pour chacun de ces *bins*, et le visualiser dans un histogramme (pour chaque saison entre 2018 et 2021).

![Image](../Q4_2_2018_2019.png)
![Image](../Q4_2_2019_2020.png)
![Image](../Q4_2_2020_2021.png)

La situation ne semble pas vraiment changer d'une saison à l'autre, mais il est clair qu'on peut détecter un 
comportement commun à chaque saison : la probabilité de faire un but décroit avec la distance initialement, 
mais commence à remonter lorsque la distance augmente. Évidemment on s'attend que lorsqu'un shot est fait de plus loin, 
il est plus difficile de faire un but. Mais qu'est-ce qui explique que les shots de *très* loin soient si efficaces ?

![Image](../Q4_2-EmptyNet.png)

La réponse se trouve avec les buts *Empty Nets*: souvent durant la dernière période, les équipes vont remplacer leur gardien de but par un autre joueur offensif, ce qui leur permet d'avoir une meilleure probabilité de marquer un dernier but. Par contre, ceci laisse aussi leur propre but vulnérable, et il est possible pour l'équipe adversaire de marquer un but à partir de leur zone défensive. En effet, si nous regardons ce graphique avec le pourcentage de *shots* qui sont des *Empty Net Goals*, nous voyons qu'il est beaucoup plus commun de marquer un but Empty Net de loin que de proche.

### 3. Graphique des types de tirs et des distances
Si maintenant nous examinons la probabilité de but dépendant du type de tir et de la distance, nous pouvons utiliser un *heatmap* pour visualiser ces données, où il y a deux dimensions discrètes d'entrée et une dimension continue de sortie.

Il suffit simplement d'exécuter une boucle 2D, pour nos deux dimensions d'entrée, et calculer le pourcentage de but pour chaque valeur.

![Image](../Q4_3.png)

En examinant le résultat, nous voyons que de très proche (0-20 ft.), c'est définitivement les shots *Tip-In* 
qui sont les plus dangereux. 
Si on va un peu plus loin (20-70 ft.), c'est d'abord les *Snap Shots* et ensuite les *Slap Shots* 
qui sont les plus efficaces. 
Encore plus loin dans la région de (110-140 ft.), les *Tip-In* et *Slap Shots* ont de bonnes chances. 
Enfin si on tire complétement de l'autre côté de la patinoire, c'est surtout les *Snap Shots* et 
aussi les *Wrist Shots* qui marchent le mieux. En conclusion, le type de shot le plus dangereux 
dépend de la distance jusqu'au but.
...
du jupyter notebook 'notebooks/Milestone-1/Q5_Visualisations_Avancees.ipynb'
