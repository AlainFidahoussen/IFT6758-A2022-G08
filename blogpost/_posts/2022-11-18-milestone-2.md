...
#### **Ingénieurie des caractéristiques**
Comme évoqué précédemment, de nouvelles caractéristiques ont été ajoutées à nos modèles avec notamment le ratio du nombre de buts sur le nombre de tirs lors de la saison précédente pour chaque tireur, le ratio du nombre de buts encaissés sur le nombre de tirs pour les gardiens de but, la préférence manuelle des joueurs (droitier ou gaucher) ainsi que la position des joueurs sur la patinoire (Défense, Centre, Aile gauche ou droite, ou gardien de but). L'idée derrière cet ajout était d'obtenir un maximum d'informations pour aider notre modèle lors de la prédiction en lui permettant ainsi d'avoir une "idée" de la qualité du tireur et si ce dernier se trouve dans une situation favorable pour marquer.
En plus de ceci, nous avons également pu réaliser du binning pour les caractéristiques de distance et d'angle toujours dans l'optique d'améliorer les performances des modèles. 

#### **Modèles testés**
En complément des modèles de regression logistique et XGBoost, de nouveaux modèles ont été testés. Notre approche au problème a été de tester un grand nombre de modèles issus de familles différentes afin de sélectionner les meilleurs d'entre eux. Ainsi voici une liste des modèles que nous avons pu utiliser: 
- Modèles utilisant les arbres de décision: Adaboost, EasyEnsembleClassifier et Random Forest
- Méthodes des plus proches voisins: KNN ou K plus proches voisins
- Réseau de neurones: Multi Layer Perceptron (MLP)

#### **Sélection des caractéristiques**
A l'instar des modèles testés, nous souhaitions implémenter diverses méthodes de sélection de caractéristiques pour nos modèles. Par conséquent, nous avons repris plusieurs méthodes vues en cours ainsi que celles proposées par la bibliothèque [ScikitLearn](https://scikit-learn.org/stable/modules/feature_selection.html){:target="_blank"}. Nous pouvons citer notamment:
- Méthodes de filtrage: Information mutuelle, Chi carré, Anova, Variance Threshold, Matrice de corrélation
- Méthodes embarquées: Lasso (norme L1) réalisé à l'aide du modèle linéaire SVC, importance basée sur Random Forest ou la regression logistique 
...
Comme évoqué de nombreuses fois, les données à notre disposition sont débalancées (90% des tirs sont des non-buts). Il était donc important de prendre en compte ce débalancement lors de la division de notre jeu de données en jeu de d'entraînement et de validation. Pour cela, l'option "stratify" (proposée par la librairie ScikitLearn) nous permet de conserver ces proportions pour chaque division que nous réalisons de notre jeu de données. Cette méthode de division a alors été utilisée pour évaluer tous nos modèles. 

#### **Ajustement des hyperparamètres, stratégies de validation croisée**

Lors de la recherche des meilleurs hyperparamètres , une méthode a été essentiellement privilégiée. En effet, si nous avons pu utiliser des méthodes comme GridSearch, nous avons essentiellement utilisé des échantillonages Bayésien qui sont bien plus efficaces et permettent d'économiser du temps et de l'énergie (comme nous avons pu le voir en cours). En complément de ceci, nous avons réalisé des validations croisées de notre jeu d'entraînement et ceci pour toutes nos recherches (généralement en divisant notre jeu d'entraînement en 5). 
Ces recherches d'hyperparamètres ont été effectuées à la fois pour les modèles que nous utilisions pour nos prédictions mais également pour les méthodes de sélection de caractéristiques, notamment pour les méthodes de filtrage afin de savoir quel nombre de caractéristiques à garder était optimal. Concernant les modèles, plusieurs plages d'hyperparamètres propres à ces derniers ont été étudiés comme la longueur, profondeur et le coefficient gamma pour les modèles usant des arbres de décision, le nombre de couches et de neurones pour les réseaux de neurones ou encore le nombre de voisins à considérer pour les modèles KNN. De manière plus générale, plusieurs valeurs de learning rate ont également été étudiées.  

#### **Régularisation**

Si l'utilisation de la norme L1 (LASSO) a été utilisée pour la sélection de caractéristiques comme vu auparavant, elle a également été utilisée dans nos modèles pour la régularisation de modèles et tout particulièrement pour les modèles MLP. Pour ces derniers, la régularisation avec la norme L2 a aussi été implémentée ainsi qu'une combinaison de ces deux normes. 
