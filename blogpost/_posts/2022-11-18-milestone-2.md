...
Comme il nous a été présenté en cours, plusieurs méthodes pour la gestion des valeurs aberrantes existent. Si dans un premier temps nous avons tenté de gérer ces valeurs au moyen de méthodes visuelles (boîtes à moustaches ou nuages de points), nous sommes rapidement tournés vers d'autres méthodes:
- IQR 
...
La distribution des données de la saison régulière 2019/2020 devrait logiquement être assez comparable à celle des autres saisons régulières sur lequelles nous avons entrainé et validé nos modèles. Ainsi, nous attendons à des performances semblables de nos modèles sur cet ensemble de test que sur l'ensemble de validation. Nous voyons effectivement cette tendance. Par exemple, pour notre modèle de random forest, nous avons obtenu un F1 macro de 0.64 sur cet ensemble, ce qui est identique au score que nous avons eu sur l'ensemble de validation. Le AUROC est aussi très similaire. Nous pouvons tirer les mêmes conclusion pour les trois modèles de régression logistique de base.
