...
def DoTraining(calibrated=False):
    ...
    clf_easyEnsemble_pca(training_data, calibrated)

    
def clf_easyEnsemble_pca(training_data, calibrated=False):
    ...
    if calibrated:
        steps = [("columns_selector", columns_selector), 
                 ...
                 ("clf_easy_ensemble",  CalibratedClassifierCV(base_estimator=clf_easy_ensemble, cv=3))]
    else:
        steps = [("columns_selector", columns_selector), 
                 ('fill_nan', fill_nan), 
                 ('one_hot', one_hot), 
                 ('scaler', scaler), 
                 ('selector', preprocessor),
                 ("clf_easy_ensemble", clf_easy_ensemble)]
    ...
    if calibrated:
        filename = 'EasyEnsemble_PCA_calibrated'
    else:
        filename = 'EasyEnsemble_PCA'
